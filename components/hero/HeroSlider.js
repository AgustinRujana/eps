...
const HeroSlider = (props) => {
  ...
  return (
    <section className='relative flex justify-center items-center h-full w-full'>
      ...
      <div className={`${props.display} ${'absolute z-40 left-1/4 text-white'}`}>
      ...
      {props.content.data.map((slide, index) => {
        return (
          <div
            className={index === current ? 'h-full opacity-100 transition-all ease-in-out duration-1000' : 'h-full opacity-0 object-cover transition-all ease-in-out duration-1000'}
            key={index}
          >
            {index === current && (
              <img src={slide.image} alt='travel' className='h-full object-cover'/>
            )}
          </div>
        );
      })}
